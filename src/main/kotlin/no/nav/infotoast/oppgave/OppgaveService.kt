...
class OppgaveService(
    ...
    private fun getOppgaveRecord(
        ...
        val beskrivelse =
            "Manuell behandling av sykmelding. Validation status: ${sykmeldingRecord.validation.status}"
        ...
        val fristFerdigstillelse =
            finnFristForFerdigstillingAvOppgave(LocalDate.now().plusDays(fristDager.toLong()))

        return OppgaveRecord(
            produserOppgave =
                ProduserOppgaveKafkaMessage(
    ...
    private fun getAktoerId(person: Person): String {
        return person.identer.find { it.gruppe == IDENT_GRUPPE.AKTORID && !it.historisk }?.ident
    ...
    private fun getBehandler(sykmeldingRecord: SykmeldingRecord): Behandler {
        return when (val value = sykmeldingRecord.sykmelding) {
            ...
            is UtenlandskSykmelding ->
                throw IllegalArgumentException("UtenlandskSykmelding has no behandler")
        }
    }
